...
class GeoGridTracker:
    ...
    def get_target_place_id(self, business_name: str, location: dict) -> str:
        """Get the canonical place_id for the target business."""
    ...
    def find_business_rank(self, serp_data: dict, business_name: str) -> dict:
        ...
        for i,r in enumerate(serp_data.get("organic_results",[]),start=1):
            if business_name.lower() in r.get('title','').lower(): org=i; break
        loc=None
        for i,r in enumerate(serp_data.get("local_results",[]),start=1):
            if business_name.lower() in r.get('title','').lower(): loc=i; break
    ...
    def run_geo_grid_tracking(self, business_name: str,
        ...
        count=0
        out=[]
        # Get canonical place_id once
        target_id = self.get_target_place_id(business_name, {'lat':center_lat,'lng':center_lng})
        for pt in points:
            for kw in keywords:
                if progress_bar: count+=1; progress_bar.progress(count/total)
    ...
    def create_folium_map(self,data: list, center_lat: float, center_lng: float, result_type: str="local_pack_rank") -> folium.Map:
        ...
        for pt in data:
            ...
            if rank is not None:
                ...
                folium.CircleMarker([lat,lng],radius=12,color=color,fill=True,fill_color=color,fill_opacity=0.7,
                                     tooltip=f"{result_type.title().replace('_',' ')}: {rank}").add_to(m)
                folium.map.Marker([lat,lng],icon=DivIcon(html=f"<div style='font-size:10px;color:white;text-align:center;'>{rank}</div>")).add_to(m)
            else:
                folium.CircleMarker([lat,lng],radius=12,color='red',fill=True,fill_color='red',fill_opacity=0.7,
                                     tooltip="Not ranked").add_to(m)
                folium.map.Marker([lat,lng],icon=DivIcon(html="<div style='font-size:10px;color:white;text-align:center;'>×</div>")).add_to(m)
        return m

    def _get_color_by_rank(self,norm: float)->str:
        if norm>=0.8: return 'green'
        if norm>=0.5: return 'yellow'
        if norm>0:   return 'orange'
        return 'red'

    def generate_summary_data(self)->dict:
        if not self.results_data: return {}
        df=pd.DataFrame(self.results_data)
        total=len(df); org=df['is_in_organic'].sum(); loc=df['is_in_local_pack'].sum(); gmap=df['gmaps_rank'].notna().sum()
        summary={'business_name':df['business_name'].iloc[0],'total_queries':total,
                 'organic_presence_pct':org/total*100,'local_presence_pct':loc/total*100,'gmaps_presence_pct':gmap/total*100}
        if org: summary['avg_organic_rank']=df.loc[df['organic_rank'].notna(),'organic_rank'].mean()
        if loc: summary['avg_local_rank']=df.loc[df['local_pack_rank'].notna(),'local_pack_rank'].mean()
        if gmap: summary['avg_gmaps_rank']=df.loc[df['gmaps_rank'].notna(),'gmaps_rank'].mean()
...
def display_grid_preview():
    if not business_address or not google_api_key:
        st.warning("Enter address & Maps key to preview grid.");return
    ...
    if grid_shape=="Circle": folium.Circle([loc['lat'],loc['lng']],radius=radius_km*1000,color='red',fill=True,fill_opacity=0.1).add_to(m)
    for p in pts: folium.CircleMarker([p['lat'],p['lng']],radius=3,color='blue',fill=True,fill_opacity=0.5).add_to(m)
    st_folium(m,width=700,height=400,key="grid_preview");st.info(f"{len(pts)} grid points generated ({grid_shape}).")

tab1,tab2,tab3,tab4=st.tabs(["Setup & Run","Maps","Summary","Raw Data"])
with tab1:
    st.header("Grid Preview");display_grid_preview()
    ...
    if st.button("Start Tracking",disabled=not all([google_api_key,serpstack_key,business_profile_name,business_address,keywords])):
        ...
        if place:
            ...
            st.session_state['results']=data;st.session_state['summary']=tracker.generate_summary_data();st.success(f"Collected {len(data)} points.")
with tab2:
    ...
    else: st.info("Run tracking to see rankings on map.")
with tab3:
    summary=st.session_state.get('summary',{})
    if summary:
        s=summary['summary'];st.metric("Total Queries",s['total_queries']);st.metric("Organic %",f"{s['organic_presence_pct']:.1f}%");st.metric("Local Pack %",f"{s['local_presence_pct']:.1f}%");st.metric("Google Maps %",f"{s['gmaps_presence_pct']:.1f}%")
    else: st.info("Summary will appear here after running tracking.")
with tab4:
    if st.session_state['results']:
        df=pd.DataFrame(st.session_state['results']);st.dataframe(df);csv=df.to_csv(index=False);st.download_button("Download CSV",data=csv,file_name="results.csv");st.download_button("Download JSON",data=json.dumps(st.session_state['results']),file_name="results.json")
    else:st.info("Raw data will appear here after running tracking.")

st.markdown("---");st.markdown("Built with Serpstack & Google Maps API")
