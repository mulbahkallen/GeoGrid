...
class GeoGridTracker:
    ...
    def search_places(self, keyword: str, location: dict, radius_m: int = 1000) -> list:
        """Use Google Maps Places Nearby Search to get ranking of places for keyword."""
    ...
    def find_business_rank(self, serp_data: dict, business_name: str) -> dict:
        """Extract organic and local-pack rank positions from Serpstack data."""
        org = None
    ...
    def find_place_rank(self, places: list, business_name: str) -> int:
        """Find the rank of a business in Google Places results."""
        for i, p in enumerate(places, start=1):
            if business_name.lower() in p.get('name', '').lower():
                return i
        return None
    ...
    def run_geo_grid_tracking(self, business_name: str,
        ...
        """Run grid + SERP + Places queries for each keyword at each point."""
        ...
        for pt in points:
            for kw in keywords:
                ...
                gmaps_rank = self.find_place_rank(places, business_name)
                rec = {
                    ...
                    **ranks,
                    ...
                    "gmaps_list": [p.get('name') for p in places[:10]]
                }
                out.append(rec)
                time.sleep(1)
    ...
    def create_folium_map(self, data: list,
        ...
        """Map with hoverable markers showing rank or X for result_type (including gmaps_rank)."""
    ...
    if st.session_state['results']:
        ...
        for idx, kw in enumerate(keywords):
            ...
            m = tr.create_folium_map(subset,
                                     ...
                                     st.session_state['results'][0]['lng'], choice)
